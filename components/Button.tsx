...
const Button = ({ buttonAction, icon, text, type, customClasses }: ButtonProps) => {
  return (
    <Pressable
      className={`flex-row items-center justify-left border rounded-lg border-dark-5 active:bg-dark-5 ${type === 'large' ? 'p-3 my-2' : 'pl-2 pr-4 py-1'} ${customClasses ? customClasses : 'bg-dark-6'}`}
      onPress={() => {
        if (!buttonAction) {
